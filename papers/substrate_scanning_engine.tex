...
% ── Metadata ──────────────────────────────────────────────────────────
\title[Substrate Scanning Engine]{Substrate Scanning Engine:\\
...
\urladdr{https://orcid.org/0009-0003-2000-0737}
\date{March 2026}
\thanks{ORCID: 0009-0003-2000-0737. Serie~I Working Paper. Code: AGPL-3.0-or-later. Papers: CC-BY-4.0.}
...
extracts 31 deterministic features per query point (29 core, 2 Euler-specific). Applied to the Riemann
...
(antenna/\penalty0 acoustic/laser
optimization), visible-light spec-tros-copy
...
(Conrad~\cite{Conrad2005}, extending earlier work of Goldfeld~\cite{Goldfeld1976} and Kurokawa
et~al.~\cite{Kurokawa1992}). The non-convergence is not a technical obstacle---it is the
fundamental structural fact. Kurokawa's ``Deep Riemann Hypothesis''~\cite{Kurokawa1992} asserts
...
The engine consists of nine modules, each independently testable. The input
is an Euler product (abstract base class); the output is a 31-dimensional
feature vector per query point $(\sigma, t, K)$: 29~core features shared
across all domains, plus 2~Euler-specific features from Module~8.
...
t}$. The embedding is standard (Bohr torus~\cite{Bohr1924}). What is specific to this
...
reference-independent), Scale~3 (CF modulation, classical Khinchin;
L\'evy rate validated at $3.42 \pm 0.39$ bits/term over 100 zeros at
100-digit precision~\cite{STA2026}). The
...
Tests independence between Channel~A (arithmetic: $v_2$, $\mathrm{ord}_2$,
$\chi$, $a_p$), Channel~B (amplitude: $\ln p$, $p^{-1/2}$), and Channel~C
...
\subsection{Module 8: Euler Phase (Accumulated Argument)}

The accumulated argument of the partial Euler product:
\[
  \mathrm{arg}(\zeta_K(s))
    = \sum_{k=1}^{K} w_k\,\mathrm{arg}(1 - p_k^{-s}),
\]
where $w_k = e^{-\pi(k/K)^2}$ is a Gaussian boundary-suppressing window.
The term ``phase'' in this module refers to the complex argument of the
local Euler factor $(1-p^{-s})$, not to the oscillatory phases
$\theta_p(t) = -t\ln p$ of Module~2 or the phase coherence of Module~4.
The distinction: $\theta_p$ is linear in~$t$ and encodes frequency;
$\mathrm{arg}(1-p^{-s})$ is non-linear and encodes the geometry of the
local factor.

Two features are extracted: the accumulated argument $\mathrm{arg}(\zeta_K)$
and its angular velocity $d(\mathrm{arg})/dt$ via central difference.  Near
a zero $\zeta(1/2+i\gamma)=0$, the argument changes sign and
$|d(\mathrm{arg})/dt|$ is maximized; away from zeros it varies smoothly.
In validation, $d(\mathrm{arg})/dt$ achieved zero/non-zero separation of
$6.4\sigma$---the highest of all 31~features.

The Gaussian window mitigates the divergence of partial Euler products on the
critical line (Conrad~\cite{Conrad2005}): the accumulated argument inherits
this divergence at the truncation boundary.  Validation showed 72--93\% CV
reduction in $K$-convergence.

A candidate third feature---the variance of argument increments across
primes---was excluded after validation revealed it to be a deterministic
function of~$\sigma$ (Spearman $\rho = 1.000$ against
$\mathrm{mean}(p^{-2\sigma})$).  The theoretical cause is Kronecker--Weyl
equidistribution of $\{t\ln p\}$ on $S^1$~\cite{CriticalCircle}: for generic~$t$,
$\mathrm{Var}(\mathrm{arg}(1-p^{-s})) \approx \tfrac{1}{2}\,E[p^{-2\sigma}]$,
reducing the observable to a proxy for~$\sigma$.  Including it would
constitute label leakage in any $\sigma$-classification task.

These two features require Euler product local factors and have no analog in
the 12~application engines, which remain at 29~core features.

\subsection{Module 9: Feature Vector}

Combines all modules into a 31-dimensional vector: 3~meta ($\sigma$, $t$,
$K$), 4~torus, 6~potential ($V$, $\nabla V$, $\Delta V$, harmonicity),
5~Weil (eigenvalues), 6~envelope ($f(b)$ stats, phase coherence),
2~Poincar\'e (radius, gap), 3~channel ($\rho$ cross-correlations),
2~Euler phase (accumulated argument, angular velocity). Output is
...
At $K=30$, $\sigma=0.5$, the four features with highest zero/non-zero
separation are: angular velocity $d(\mathrm{arg})/dt$ from Module~8
($6.4\sigma$), phase coherence ($5.6\sigma$), $V$~potential ($2.4\sigma$),
and gradient magnitude ($0.75\sigma$). Phase coherence alone classifies zeros
with zero training. This is the Channel~C signal: at a zero, the phases
$\ln(p)\cdot\gamma$ conspire, producing high coherence; away from zeros, they
cancel. The angular velocity achieves stronger separation because near a zero
the argument of $\zeta$ changes sign, maximizing $|d(\mathrm{arg})/dt|$;
away from zeros the argument varies smoothly.

\subsection{Envelope Factorization}

Per-zero optimal weights are \textsc{zero-specific} (mean pairwise
$|r| = 0.14$). The third axis beyond Channel~A and~B is
$\cos(\ln(p)\cdot\gamma)$---the phase of the target zero evaluated at each
prime ($\rho = -0.59$, $p = 6.6\times 10^{-4}$ at $K=30$). Channel~A ($v_2$,
...
$-1.23$). Channel~A ($a_p$) is orthogonal to all geometric features
...
\item The toroidal embedding via $p^{-s}$ on the Bohr torus is classical
  (Bohr~\cite{Bohr1924}). Our contribution: the pair-weighted mixed-torus
...
\item The class number identity $S - k/2 = -h(-p)/2$ is derivable from
  Dirichlet~\cite{Dirichlet1839}. Our contribution: the route of discovery (Substrate
...
The engine generates 31 geometric features per $(\sigma, t)$ query for any
...
exactly to Channel~B, Channel~A, and cross-channel respectively. The motor
...
images follow $1/f^2$, per Field~\cite{Field1987}), band energies, peak detection,
...
engine generates 29~features from group order factorization (Channel~A: largest
...
order is the spectral gap, factorization structure is Channel~A, embedding
...
\item \texttt{geometric\_engine.py} --- self-contained Python engine
  (numpy/scipy only), 9~modules, 31~features (29~core + 2~Euler-specific),
  ${\sim}8\,$ms per query.
...
All engines share the same 29-feature core architecture; the core engine
produces 31 (29~core + 2~Euler-specific from Module~8).
...
The phrase that closes the arc: $t$ parametrizes the critical line.
...
phases conspire. $f(b)$ organizes the amplitude. $\cos(\omega)$ organizes
the phase. Channel~A is orthogonal to both. Everything in this paper---the
embedding, the three scales, the potential, the 16~domains, the motor---is
scaffolding around this single factorization.

A note on democratization. The engine lowers the barrier of access: anyone
...
confidence. The real democratization in this paper is not the motor---it
...
\bibitem{STA2026}
R.~Hern\'andez Reveles,
\emph{Technical Report: Autonomous Traversal Synchrony Algorithm (STA)},
Serie~I, v2 (2026). DOI: 10.5281/zenodo.19353699.
% Validates L\'evy rate at 3.42 bits/term for 100 zeta zeros (pilot).
% Operates exclusively in Channel~C of the SSE framework.

\bibitem{CriticalCircle}
R.~Hern\'andez Reveles,
\emph{The Critical Circle: a Topological Cartography of the Critical
  Line via the Canonical Logarithm},
Serie~I (2026). DOI: 10.5281/zenodo.19104306.
% Theorem 2.1: unconditional, from ln + TFA + Kronecker--Weyl.
% Corollary 4.1: zeros strictly complex in b-world.

\bibitem{Bohr1924}
H.~Bohr,
\emph{Zur Theorie der fastperiodischen Funktionen~I--III},
Acta Mathematica \textbf{45}--\textbf{47} (1924--1926).

\bibitem{Dirichlet1839}
P.\,G.\,L.~Dirichlet,
Recherches sur diverses applications de l'analyse infinit\'esimale
\`a la th\'eorie des nombres,
\emph{J.\ reine angew.\ Math.}\ \textbf{19} (1839), 324--369.

\bibitem{Field1987}
D.\,J.~Field,
Relations between the statistics of natural images and the response
properties of cortical cells,
\emph{J.\ Opt.\ Soc.\ Am.\ A} \textbf{4} (1987), no.~12, 2379--2394.

\bibitem{Goldfeld1976}
D.~Goldfeld,
The nonvanishing of $L(1,\chi)$ and the Siegel zero (after
Iwaniec--Kowalski),
in \emph{Analytic Number Theory} (Y.~Motohashi, ed.),
London Math.\ Soc.\ Lecture Note Ser.\ \textbf{247},
Cambridge Univ.\ Press, 1997, pp.~141--172.

\bibitem{Kurokawa1992}
N.~Kurokawa,
On the meromorphy of Euler products,
\emph{Proc.\ London Math.\ Soc.}\ (3) \textbf{53} (1992), 1--47.
